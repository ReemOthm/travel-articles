...
const SingleArticle = async({ params}: {params: Promise<{ slug: string }> })=>{
    ...
    const data = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/articles/${(await params).slug}`).then(res=> res.json())
    ...
    return (
        <div className="pt-8 md:w-1/2 md:m-auto min-h-[78vh] mx-3">
            <Suspense fallback={<div className="bg-[#cccccc7a] p-3 rounded-md mb-5 h-96"></div>}>
